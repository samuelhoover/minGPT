...
class Encoder:
    ...
    def encode_and_show_work(self, text):
        """Debuggin function, same as `encode` but returns all intermediate work."""
        bpe_idx = []
        parts = []
        tokens = re.findall(self.pat, text)
        for token in tokens:
...
def get_encoder():
    ...
    assert (
        len(encoder) == 50257
    )  # 256 individual byte tokens, 50,000 merged tokens, 1 special <|endoftext|> token
...
class BPETokenizer:
    ...
    def __call__(self, text, return_tensors="pt"):
        assert return_tensors == "pt"  # PyTorch only
        assert isinstance(text, str)  # single string input
...
if __name__ == "__main__":
    text = "Hello! I'm Sam Hoover and it's 2024. w00t :D 🤗"
    e = get_encoder()
